...
│   │   │   ├── DocumentSelector.tsx # 规范（文档）多选下拉（V1.2.5）
...
2. **指定规范**（V1.2.5，可选）：知识库右侧「规范选择」下拉可勾选一本或多本规范，将检索范围限定到所选文档；不选则搜整个知识库。切换知识库时已选规范自动清空
3. 输入问题，按 Enter 或点击发送
4. AI 基于知识库内容回答，并标注引用来源
5. **连续对话**（V1.2.3）：同一会话内的历史消息自动注入上下文，支持多轮追问
6. **图片展示**（V1.0.9）：当检索结果包含相关图片时：
   - 大模型会在回答的对应位置内嵌图片
   - 若未内嵌，后端自动追加「相关图片」小节
   - 保证图片相关的查询必然展示图片
...
3. 输入报告标题（用于 docx 文件名与首页标题）
4. 在输入框中粘贴报告/参考资料文本
5. 点击附件按钮上传图片或文档（docx/txt/md/pdf），大文档自动分块阅读
6. 输入 `/` 选择编制技能（skill），选中后注入 system prompt 指令
7. 点击发送，AI 先输出报告要点（流式显示），再静默生成完整报告
8. 可展开查看完整报告、下载 docx、保存到左侧历史记录
...
- 🔍 **BM25 候选池重构**（`_merge_bm25_pool`）：分词级子串仅作召回补漏（不再覆盖/抬高已打分块，防关键字重复刷榜），原始整句字面命中强提权（`SUBSTRING_PHRASE_WEIGHT=5x`，对齐预览搜索高亮）
- ⚖️ **RRF 权重语义偏重**：默认 BM25 0.35 / 向量 0.65，降低关键字重复块刷榜影响；精确字面命中靠整句提权兜底
- 🛡️ **空值 env 容错**：`env_ignore_empty=True`，docker-compose 纯透传 `${VAR:-}` 时空串不炸 int/float/bool 字段，默认值单一来源在 config.py

### V1.2.4 (2026-08-05)
- 🔍 **检索召回率调优**：新增配置项 `RAG_TOP_K=8`、`VECTOR_SCORE_THRESHOLD=0.2`、`ENABLE_SUBSTRING_BOOST`、`SUBSTRING_PHRASE_WEIGHT`、`ENABLE_LITERAL_FORCE_INJECT`
- 🛠️ 字面命中强制回插兜底（`_inject_literal_hit`，默认关），确保精确表名/条款号类字面块进入 LLM 上下文
