/**
 * PPT 制作页面（V1.2.1+；V1.2.6 起接入 ppt-master 容器）。
 *
 * 文本 + 附件文档（docx/txt/md/pdf） + 可选知识库 → 提交后端 → 后端调 ppt-master
 * 容器（Claude Code + ppt-master skill）端到端生成原生可编辑 PPTX → 前端流式展示
 * 进度并下载 .pptx；左侧保存已生成 PPT 的历史记录，可回看详情并重新下载。
 */
...
export default function Ppt() {
  ...
  /* ----- 发送：历史 + 附件文档 → 调 ppt-master 容器生成 PPT ----- */
    ...
    streamRef.current = streamPptChat({
      ...
      onError: (err) => {
        setMessages((prev) =>
          prev.map((m) =>
            m.id === aiId
              ? { ...m, status: 'error', error: err, content: `生成失败：${err}` }
              : m,
          ),
        )
        setStreaming(false)
  ...
  const downloadFile = async (fileId: string, dlTitle: string) => {
    if (exporting) return
    setExporting(true)
    setExportMsg('')
    try {
      const blob = await downloadPptxFile(fileId)
      triggerBlobDownload(blob, `${dlTitle || 'PPT演示'}.pptx`)
  ...
  const saveCurrentPpt = async () => {
    ...
    try {
      await savePptRecord(title.trim() || 'PPT', lastFileId, lastUserQuestion)
              ...
              <div className="chat-topbar">
                ...
                <div className="topbar-status">
                  ...
                  <button
                    ...
                    type="button"
                    disabled={exporting}
                    onClick={() => downloadFile(currentRecord.content, currentRecord.title)}
                  >
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="chat-messages" ref={messagesRef}>
                <div className="chat-column">
                  <div className="report-record-card">
                    ...
                    <div className="report-record-question-text">
                      该 PPT 已生成，请点击右上角「下载pptx」下载。
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="chat-topbar">
                ...
                <div className="topbar-status">
                  <button
                    ...
                    type="button"
                    disabled={!lastFileId || saving || streaming}
                  ...
                  <button
                    ...
                    onClick={() => lastFileId && downloadFile(lastFileId, title.trim() || 'PPT演示')}
                  >
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                      <polyline points="7 10 12 15 17 10" />
                    ...
                    <div className="welcome-bubble">
                      ...
                      <div className="welcome-text">
                        {selectedKb
                          ...
                          : `你好！我是${APP_NAME} ${APP_VERSION} PPT 制作助手。当前未选择知识库，将仅基于您输入的文本制作 PPT。请输入内容，点击发送后生成可下载的原生 PPT。`}
                      </div>
                    </div>
                  )}
                        ...
                        <div className="message-content">
                          <div className="bubble ai-bubble">
                            {msg.status === 'thinking' ? (
                              ...
                              <div className="ai-answer">
                                <span style={{ color: '#dc2626', fontSize: '13px' }}>{msg.error || msg.content || '生成失败'}</span>
                              </div>
                            ) : (
                              <div className="ai-answer">
                                {pptFiles[msg.id] ? (
          ...
          <div className="input-column">
            ...
            <div className="input-wrapper" ref={inputWrapRef}>
              ...
              <textarea
                ...
                placeholder={selectedKb ? '输入要制作 PPT 的文本，可上传 docx/txt/md/pdf 作为素材，Enter发送...' : '未选择知识库，仅凭输入文本制作 PPT。可上传 docx/txt/md/pdf，Enter发送...'}
                rows={1}
                ref={textareaRef}
                value={input}
